...
describe(suite.title, async function () {
    ...
    function findSegment(segmentName: string) {
    ...
    describe('happy day: Search', async function () {
        let commandBox: Element, button: HTMLInputElement;
        ...
        it('interact', async function () {
            const input: HTMLInputElement = commandBox.querySelector('.command-text-input')!;
            CT.input('goat', input);
        });
        it('looks good', async function () {
            this.retries(64);
...
function visibleGames(withDeck: Element) {
