...
(function (Skyward) {
     ...
     */
    function fly(target, { type, detail }) {
        return target.dispatchEvent(Object.assign(new CustomEvent(type, { detail, bubbles: true, composed: true }), { originatingTarget: target }));
    }
    Skyward.fly = fly;
    /**
     ...
     */
    function stage(event, target, { type, detail }) {
     ...
     */
    function reentry(event, { type, detail }) {
        const target = event.originatingTarget;
        if (!target) {
            throw new Error("No originating target found for event reentry");
        }
        return target.dispatchEvent(new CustomEvent(type, { detail }));
    }
    Skyward.reentry = reentry;
})(Skyward || (Skyward = {}));
//# sourceMappingURL=Skyward.js.map