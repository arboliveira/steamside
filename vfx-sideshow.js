export class Sideshow {
    ...
    static customary_dispatchEvent_Customary_lifecycle_firstUpdated(sender) {
        sender.dispatchEvent(new CustomEvent('CustomaryEvent:lifecycle:firstUpdated', {
            detail: { dispatcher: sender },
            composed: true,
            bubbles: true,
        }));
    }
    on_firstUpdated_Event(event) {
        const dispatcher = event.detail.dispatcher;
        if (!this.#performers.includes(dispatcher))
            return;
        this.#performers_ready++;
        if (this.#performers_ready !== this.#performers.length)
            return;
        let left = true;
...
//# sourceMappingURL=vfx-sideshow.js.map